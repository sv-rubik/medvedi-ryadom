...
REGION_ADJECTIVES = {
    'Амурск': 'Амурская область', 'Архангельск': 'Архангельская область',
    'Астраханск': 'Астраханская область', 'Белгородск': 'Белгородская область',
    'Брянск': 'Брянская область', 'Владимирск': 'Владимирская область',
    'Волгоградск': 'Волгоградская область', 'Вологодск': 'Вологодская область',
    'Воронежск': 'Воронежская область', 'Ивановск': 'Ивановская область',
    'Иркутск': 'Иркутская область', 'Калининградск': 'Калининградская область',
    'Калужск': 'Калужская область', 'Кемеровск': 'Кемеровская область',
    'Кировск': 'Кировская область', 'Костромск': 'Костромская область',
    'Курганск': 'Курганская область', 'Курск': 'Курская область',
    'Ленинградск': 'Ленинградская область', 'Липецк': 'Липецкая область',
    'Магаданск': 'Магаданская область', 'Московск': 'Московская область',
    'Мурманск': 'Мурманская область', 'Нижегородск': 'Нижегородская область',
    'Новгородск': 'Новгородская область', 'Новосибирск': 'Новосибирская область',
    'Омск': 'Омская область', 'Оренбургск': 'Оренбургская область',
    'Орловск': 'Орловская область', 'Пензенск': 'Пензенская область',
    'Псковск': 'Псковская область', 'Ростовск': 'Ростовская область',
    'Рязанск': 'Рязанская область', 'Самарск': 'Самарская область',
    'Саратовск': 'Саратовская область', 'Сахалинск': 'Сахалинская область',
    'Свердловск': 'Свердловская область', 'Смоленск': 'Смоленская область',
    'Тамбовск': 'Тамбовская область', 'Тверск': 'Тверская область',
    'Томск': 'Томская область', 'Тульск': 'Тульская область',
    'Тюменск': 'Тюменская область', 'Ульяновск': 'Ульяновская область',
    'Челябинск': 'Челябинская область', 'Ярославск': 'Ярославская область',
    'Алтайск': 'Алтайский край', 'Забайкальск': 'Забайкальский край',
    'Камчатск': 'Камчатский край', 'Краснодарск': 'Краснодарский край',
    'Красноярск': 'Красноярский край', 'Пермск': 'Пермский край',
    'Приморск': 'Приморский край', 'Ставропольск': 'Ставропольский край',
    'Хабаровск': 'Хабаровский край',
}
REGION_FORM = re.compile(r'\b([А-ЯЁ][а-яё-]+?)(?:ой|ей|ом|им)\s+(области|крае)', re.I)
BEAR = re.compile(r'\bмедвед(?:ь|я|ю|ем|и|ей|ям|ями|ях)|\bмедвеж(?:ий|ья|ьи|ьего|ьей|ьих)', re.I)
ENCOUNTER = re.compile(
    r'напад|напал|напала|напали|атаков|покус|загрыз|задр|растерза|ранил|погиб|убил|покалечил|жертв|'
    ...
    r'преслед|отпуг|прогнал|загнал|осаду', re.I
)
ATTACK = re.compile(r'напад|напал|напала|напали|атаков|покус|загрыз|задр|растерза|ранил|погиб|убил|покалечил|жертв', re.I)
REJECT = re.compile(
    r'медведев|зоопарк|цирк|мультфильм|маша и медвед(?:ь|и)|игрушечн|'
    r'бирж|рынк|акци|крипто|панда|спектакл|'
    r'спасли медвед|медвед(?:ь|я) (?:погиб|нашли мертв)|тушу медведя обнаруж|'
...
EDITORIAL = re.compile(
    r'\bпочему\b|\bмнение\b|объяснил|что происходит|как себя вести|'
    r'\bкарта\b|где встречали|ситуац|численност|статистик|'
    r'за (?:год|недел|месяц|сутки)|сообщени[йя]|случаев|'
    r'ввели (?:режим|огранич)|огранич(?:ат|или)|меры безопасности|'
    r'расслед|возбудили дело|\bотстрел\b|разрешили|\bквот[уы]\b|'
    r'представлявш[а-я]+ угрозу|\bликвидировали\s+\d+|'
    r'по всей россии|\bсвыше\s+\d+|\bболее\s+\d+|более десятка|'
    r'\bчто известно\b|\bгде\b.*\bвидели\b|какая обстановка|'
    r'тем временем|\bнасчитали\b|\bэко-патрули\b|усиливают защиту|'
    r'вышли из берлог|\bдесятки выходов\b|\bследы обнаружены\b', re.I
)
...
def relevant(title: str) -> bool:
    return bool(BEAR.search(title) and ENCOUNTER.search(title)
                ...
                and not GENERAL_NEWS.search(title) and not EDITORIAL.search(title))
...
def location_candidates(title: str) -> list[str]:
    candidates = []
    lowered = title.lower()
    for match in REGION_FORM.finditer(title):
        stem = match.group(1).capitalize()
        kind = 'область' if match.group(2).lower() == 'области' else 'край'
        normal = REGION_ADJECTIVES.get(stem)
        if normal and normal.endswith(kind):
            candidates.append((match.start(), normal))
    ...
    for match in PLACE_PATTERN.finditer(title):
        ...
        if name.split()[0] not in BAD_PLACES:
            candidates.append((match.start(1), name))
